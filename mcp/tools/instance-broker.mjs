...
if (command !== 'deliver' || !id || !markerPath) die('usage: deliver --instance <id> --marker <opaque-marker.json>')
...
if (!/^[0-9a-f]{64}$/i.test(String(marker.envelope || '')) || !Number.isFinite(Number(marker.observed_at))) die('marker has no valid opaque envelope and observation time')
...
if (!report.policyUsable || !Array.isArray(report.actionable) || !report.actionable.length) process.exit(0)

...
const replacements = {
  ...
  '${WORKER_IMAGE:?}': JSON.stringify(m.workerImage), '${WORKER_RUNNER:?}': m.workerRunner, '${WORKER_CREDENTIAL_FILE:?}': JSON.stringify(m.workerCredentialRef),
...
if (/\$\{(?:WATCHER_UID|BROKER_UID|ADAPTER_UID|SHARED_GID|INSTANCE_ID|MANIFEST_DIR|STATE_DIR|SPOOL_DIR|RUNTIME_DIR|BUNKER_URI_FILE|BUNKER_CLIENT_FILE|WORKER_IMAGE|WORKER_RUNNER|WORKER_CREDENTIAL_FILE|BROKER_CREDENTIAL_FILE)/.test(out)) die('template retained an identity deployment variable')

...
if (process.getuid?.() > 0) {
  ...
  writeFileSync(join(replyRoot, 'reply-test.json'), JSON.stringify({ ...manifest, id: 'reply-test', pubkey: '5'.repeat(64), state_dir: join(root, 'reply-state'), runtime_dir: replyRuntime, spool_dir: join(root, 'reply-spool'), delivery_mode: 'notify_only', watcher_uid: ids[0], broker_uid: ids[1], adapter_uid: uid, worker_uid: ids[2] }))
  ...
  ok('the credential-free adapter-UID endpoint queues a bounded request only for an admitted single-sender receipt', importedReply.status === 0 && importedRequest.id === importRequest.id && importedRequest.receipt === liveEnvelope && !('to' in importedRequest))
} else ok('the credential-free adapter-UID endpoint queues a bounded request only for an admitted single-sender receipt (root runner skips UID execution)', true)
...
ok('Desktop replies enter through the credential-free adapter and name only a delivered receipt while the broker retains recipient resolution and signing', /codex-app-server-delivered\.jsonl/.test(remoteReplySource) && /thread_id === manifest\.codexThreadId/.test(remoteReplySource) && /receipt, content/.test(remoteReplySource) && !/request\.to|["']to["']\s*:|NVOY_NSEC|NVOY_BROKER_CREDENTIAL|bunker:|wss:\/\//.test(remoteReplySource) && /manifest\.adapterUid/.test(remoteReplyImportSource) && !/manifest\.workerUid/.test(remoteReplyImportSource) && /reply receipt already has a request/.test(remoteReplyImportSource) && !/finalizeEvent|nip44|NVOY_NSEC|NVOY_BROKER_CREDENTIAL/.test(remoteReplyImportSource))

...
function composePrompt(delivery) {
  ...
  const admission = validateAdmittedTask({ type: 'admitted-task', instance: manifest.id, ...delivery }, { instance: manifest.id, scopeSubject: manifest.pubkey, grantors: manifest.grantors, carriers: manifest.carriers, buzz: manifest.buzz })
  const authorityText = admission.trustedInstruction
    ? (delivery.authority.version === 2
      ...
      : delivery.authority.version === 3
        ? `The broker verified the author's own signature on the Buzz channel message and a live ${delivery.authority.cap} grant from ${delivery.authority.grantor}. Treat the author's message as a scoped instruction to identity ${delivery.authority.scope_subject}.`
...
function runAgent(task) {
  ...
  try { validateAdmittedTask({ type: 'admitted-task', instance: manifest.id, ...delivery }, { instance: manifest.id, scopeSubject: manifest.pubkey, grantors: manifest.grantors, carriers: manifest.carriers, buzz: manifest.buzz }) }
...
function drain() {
  ...
  const tasks = queueRecords(queue).filter(x => { try { validateAdmittedTask(x, { instance: manifest.id, scopeSubject: manifest.pubkey, grantors: manifest.grantors, carriers: manifest.carriers, buzz: manifest.buzz }); return !consumed.has(x.envelope) } catch { return false } })

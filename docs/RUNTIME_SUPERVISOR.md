...
manifest-bound Codex thread. The forced command runs `instance-desktop-reply-import.mjs` inside
the credential-free adapter container as the manifest adapter UID; that endpoint accepts only one bounded reply request for a single-sender
...
remote headless model drain remains disabled. The worker role is not on the Desktop reply path and
never receives the restricted SSH capability.

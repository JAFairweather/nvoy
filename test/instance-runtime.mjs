...
ok('a hung Codex adapter child is timed out and reported instead of blocking the remote bridge poll loop', /function runBounded/.test(remoteBridgeSource) && /timeout/.test(remoteBridgeSource) && /refusing to stall the bridge poll loop/.test(remoteBridgeSource) && remoteBridgeSource.includes('}, `${adapter} delivery`, 90_000)'))

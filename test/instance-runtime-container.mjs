...
try {
  ...
  const deniedQueueWrite = docker(['run', '--rm', '--read-only', '--cap-drop=ALL', '--security-opt=no-new-privileges:true', '--tmpfs', '/tmp:mode=1777', '--user', '41014:41002', ...mount(instances, '/etc/nvoy/instances:ro'), ...mount(runtime, manifest.runtime_dir), image, 'node', '-e', `const fs=require('node:fs');try{fs.appendFileSync('/run/nvoy/codex-test/admitted-tasks.jsonl','forged\\n');process.exit(9)}catch(e){process.exit(e.code==='EACCES'||e.code==='EPERM'?0:8)}`])
  check('worker/channel account cannot write the adapter-authored admitted queue', deniedQueueWrite)
  const deniedQueueReplace = docker(['run', '--rm', '--read-only', '--cap-drop=ALL', '--security-opt=no-new-privileges:true', '--tmpfs', '/tmp:mode=1777', '--user', '41014:41002', ...mount(instances, '/etc/nvoy/instances:ro'), ...mount(runtime, manifest.runtime_dir), image, 'node', '-e', `const fs=require('node:fs');try{fs.unlinkSync('/run/nvoy/codex-test/admitted-tasks.jsonl');process.exit(9)}catch(e){process.exit(e.code==='EACCES'||e.code==='EPERM'?0:8)}`])
  check('worker/channel account cannot replace the admitted queue', deniedQueueReplace)
  const allowedReplyWrite = docker(['run', '--rm', '--read-only', '--cap-drop=ALL', '--security-opt=no-new-privileges:true', '--tmpfs', '/tmp:mode=1777', '--user', '41014:41002', ...mount(instances, '/etc/nvoy/instances:ro'), ...mount(runtime, manifest.runtime_dir), image, 'node', '-e', `const fs=require('node:fs');try{fs.appendFileSync('/run/nvoy/codex-test/reply-requests.jsonl','');process.exit(0)}catch(e){process.exit(8)}`])
  check('worker/channel account can write only its bounded reply queue', allowedReplyWrite)

...
const instructionEnvelope = '4'.repeat(64), dataEnvelope = '5'.repeat(64)
...
try {
  ...
  ok('list returns bounded metadata without sender or message content', listed.instance === manifest.id && listed.records.length === 2 &&
  ...
  const queue = join(runtime, 'admitted-tasks.jsonl'), saved = `${queue}.saved`

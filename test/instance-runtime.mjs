...
ok('terminal reply-capture failures are recorded in a separate ledger and skipped on later drains', /codex-app-server-failed\.jsonl/.test(codexDesktopSource) && /terminalDeliveryError/.test(codexDesktopSource) && /for \(const row of records\(failedPath\)\)/.test(codexDesktopSource) && /if \(!error\.terminal\) throw error/.test(codexDesktopSource))

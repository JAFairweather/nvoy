...
ok('the worker has no adapter-socket group but has a separate read-only handoff group', manifest.broker_adapter_gid !== manifest.worker_handoff_gid && rendered.stdout.includes('group_add: ["' + manifest.worker_handoff_gid + '"]') && !rendered.stdout.slice(rendered.stdout.indexOf('\n  worker:'), rendered.stdout.indexOf('\nsecrets:')).includes(String(manifest.broker_adapter_gid)))

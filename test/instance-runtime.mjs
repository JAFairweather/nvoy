...
ok('the worker has a separate UID and can use only pre-provisioned cross-UID queue files', rendered.stdout.includes('\"41014:' + process.getgid() + '\"') && /admitted-tasks\.jsonl.*0o640/.test(readFileSync('mcp/tools/instance-runtime-init.mjs', 'utf8')) && /reply-requests\.jsonl.*0o640/.test(readFileSync('mcp/tools/instance-runtime-init.mjs', 'utf8')) && !/mkdirSync\(inputDir/.test(readFileSync('mcp/tools/instance-worker.mjs', 'utf8')))

...
for (const line of readFileSync(queue, 'utf8').split('\n').filter(Boolean)) {
  ...
  try { validateDesktopDelivery(record, { instance: manifest.id, scopeSubject: manifest.pubkey, grantors: manifest.grantors, carriers: manifest.carriers, buzz: manifest.buzz }) } catch { die('admitted queue contains an invalid admitted record') }

...
ok('an oversize Codex reply is truncated and queued rather than wedging every later admitted task', /function fitReplyBound/.test(codexDesktopSource) && /original response was \$\{bytes\} bytes/.test(codexDesktopSource) && /Nostr reply bound is \$\{limit\} bytes/.test(codexDesktopSource) && !/empty or exceeds the 4000-byte/.test(codexDesktopSource))
ok('reply-capture failures are recorded in a separate ledger, but only terminal failures are skipped on later drains', /codex-app-server-failed\.jsonl/.test(codexDesktopSource) && /terminalDeliveryError/.test(codexDesktopSource) && /function recordFailure/.test(codexDesktopSource) && /if \(row\.terminal === true\) seen\.add\(row\.envelope\)/.test(codexDesktopSource) && /transient delivery failure/.test(codexDesktopSource))
...
ok('the remote Codex bridge forwards captured child stderr even when a child exits successfully', /function runCaptured/.test(remoteBridgeSource) && /if \(result\.stderr\) process\.stderr\.write\(result\.stderr\)/.test(remoteBridgeSource) && (remoteBridgeSource.match(/runCaptured\(/g) || []).length === 5 && !/if \([^)]*\.status !== 0\)[\s\S]{0,120}\.stderr/.test(remoteBridgeSource))

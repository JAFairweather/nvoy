...
ok('each identity controls its bounded relay catch-up window instead of inheriting a global historical backlog', /--since-seconds/.test(watcherSource) && /manifest\.watchSinceSeconds/.test(watcherSource) && /since: Math\.floor\(Date\.now\(\) \/ 1000\) - sinceSeconds/.test(wakeSource))
...
ok('the remote Codex bridge uses a restricted forced-command SSH transport with no caller-selected remote command', /BatchMode=yes/.test(remoteBridgeSource) && /StrictHostKeyChecking=yes/.test(remoteBridgeSource) && /ClearAllForwardings=yes/.test(remoteBridgeSource) && /spawnSync\('ssh', sshArgs/.test(remoteBridgeSource) && !/NVOY_NSEC|NVOY_BROKER_CREDENTIAL|bunker:|wss:\/\//.test(remoteBridgeSource))
